...
# --- XRAY PERMISSION SAFETY ---
# Xray official systemd service runs as user "nobody" on this installer.
# Any jq/mktemp -> mv replacement must preserve read access for that user.
# This helper is intentionally idempotent and is called after every config replacement.
fix_xray_permissions() {
    mkdir -p "$XRAY_DIR" /var/log/xray
    chown root:nobody "$XRAY_DIR" 2>/dev/null || true
    chmod 755 "$XRAY_DIR" 2>/dev/null || true

    if [[ -f "$CONFIG_FILE" ]]; then
        chown root:nobody "$CONFIG_FILE" 2>/dev/null || true
        chmod 640 "$CONFIG_FILE" 2>/dev/null || true
    fi

    if [[ -f "$XRAY_DIR/xray.key" ]]; then
        chown root:nobody "$XRAY_DIR/xray.key" 2>/dev/null || true
        chmod 640 "$XRAY_DIR/xray.key" 2>/dev/null || true
    fi

    if [[ -f "$XRAY_DIR/xray.crt" ]]; then
        chown root:nobody "$XRAY_DIR/xray.crt" 2>/dev/null || true
        chmod 644 "$XRAY_DIR/xray.crt" 2>/dev/null || true
    fi

    touch /var/log/xray/access.log /var/log/xray/error.log 2>/dev/null || true
    chown root:nobody /var/log/xray/access.log /var/log/xray/error.log 2>/dev/null || true
...
(
    ...
    chown root:nobody $XRAY_DIR/xray.key $XRAY_DIR/xray.crt 2>/dev/null || true
    chmod 640 $XRAY_DIR/xray.key; chmod 644 $XRAY_DIR/xray.crt
    chown root:nobody /var/log/xray/access.log /var/log/xray/error.log 2>/dev/null || true
    chmod 664 /var/log/xray/access.log /var/log/xray/error.log 2>/dev/null || true
) >/dev/null 2>&1 & install_spin
print_ok "Domain & SSL"
...
fix_xray_permissions() {
    chown root:nobody "$CONFIG" 2>/dev/null || true
...
_tf1=$(mktemp) &&                 jq --arg u "$user" '(.inbounds[] | select(.protocol == "'$proto'")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf1" && mv "$_tf1" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
                sed -i "/^$user|/d" $FILE
...
_tf2=$(mktemp) &&                     jq --arg p "$id" --arg u "$user" '(.inbounds[] | select(.protocol == "trojan")).settings.clients += [{"password":$p,"email":$u,"level":0}]' $CONFIG > "$_tf2" && mv "$_tf2" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
                else
_tf3=$(mktemp) &&                     jq --arg id "$id" --arg u "$user" '(.inbounds[] | select(.protocol == "'$proto'")).settings.clients += [{"id":$id,"email":$u,"level":0}]' $CONFIG > "$_tf3" && mv "$_tf3" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
                fi
...
_tf4=$(mktemp) &&             jq --arg u "$user" '(.inbounds[] | select(.protocol == "'$proto'")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf4" && mv "$_tf4" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
...
_tf5=$(mktemp) &&             jq --arg u "$user" '(.inbounds[] | select(.protocol == "'$proto'")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf5" && mv "$_tf5" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
            sed -i "/^$user|/d" "$FILE"
            rm -f "$QUOTA_FILE"
            fix_xray_permissions
            systemctl restart xray
...
fix_xray_permissions() {
    ...
    chown root:nobody "$(dirname "$CONFIG")" 2>/dev/null || true
    chmod 755 "$(dirname "$CONFIG")" 2>/dev/null || true
    chown root:nobody "$CONFIG" 2>/dev/null || true
    chmod 640 "$CONFIG" 2>/dev/null || true
    if [[ -f "$(dirname "$CONFIG")/xray.key" ]]; then
        chown root:nobody "$(dirname "$CONFIG")/xray.key" 2>/dev/null || true
        chmod 640 "$(dirname "$CONFIG")/xray.key" 2>/dev/null || true
    fi
    if [[ -f "$(dirname "$CONFIG")/xray.crt" ]]; then
        chown root:nobody "$(dirname "$CONFIG")/xray.crt" 2>/dev/null || true
        chmod 644 "$(dirname "$CONFIG")/xray.crt" 2>/dev/null || true
    fi
}
    ...
    while true; do header_sub
        ...
        case $opt in
            1) read -p " Username : " u; [[ -z "$u" ]] && continue
               ...
               exp_date=$(date -d "+$ex days" +"%Y-%m-%d");_tf6=$(mktemp) &&  jq --arg u "$u" --arg id "$id" '(.inbounds[] | select(.protocol == "vmess")).settings.clients += [{"id":$id,"email":$u,"level":0}]' $CONFIG > "$_tf6" && mv "$_tf6" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; echo "$u|$id|$exp_date|$limit|ACTIVE|$quota" >> $D_VMESS; echo "0 0" > "/usr/local/etc/xray/quota/$u"
               ...
               ;;
            2) nl $D_VMESS; read -p "No: " n; [[ -z "$n" ]] && continue; u=$(sed -n "${n}p" $D_VMESS | cut -d'|' -f1); sed -i "${n}d" $D_VMESS;_tf7=$(mktemp) &&  jq --arg u "$u" '(.inbounds[] | select(.protocol == "vmess")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf7" && mv "$_tf7" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; rm -f "/usr/local/etc/xray/quota/$u"
               echo -e "${GREEN}Account Deleted!${NC}"; fix_xray_permissions; systemctl restart xray >/dev/null 2>&1; sleep 2;;
            3) nl $D_VMESS; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_VMESS); u=$(echo "$line" | cut -d'|' -f1); id=$(echo "$line" | cut -d'|' -f2); exp_old=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); quota=$(echo "$line" | cut -d'|' -f6); read -p " Add Days: " add_days; exp_new=$(date -d "$exp_old + $add_days days" +"%Y-%m-%d"); sed -i "${n}s/.*/$u|$id|$exp_new|$limit|$stat|$quota/" $D_VMESS
               echo -e "${GREEN}Account $u Renewed until $exp_new!${NC}"; fix_xray_permissions; systemctl restart xray >/dev/null 2>&1; sleep 2;;
            ...
            5) u="trial-$(tr -dc a-z0-9 </dev/urandom | head -c 5)"; echo -e " Username (Trial): ${GREEN}$u${NC}"; p="$u"; id="$p"; read -p " Duration (e.g., 10m, 1h): " dur;
               if [[ "$dur" == *m ]]; then add_str="+${dur%m} minutes"; elif [[ "$dur" == *h ]]; then add_str="+${dur%h} hours"; else add_str="+1 hours"; fi
               exp_date=$(date -d "$add_str" +"%Y-%m-%d %H:%M:%S"); limit=1; quota=0;_tf8=$(mktemp) &&  jq --arg u "$u" --arg id "$id" '(.inbounds[] | select(.protocol == "vmess")).settings.clients += [{"id":$id,"email":$u,"level":0}]' $CONFIG > "$_tf8" && mv "$_tf8" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; echo "$u|$id|$exp_date|$limit|ACTIVE|$quota" >> $D_VMESS; echo "0 0" > "/usr/local/etc/xray/quota/$u"
               DMN=$(cat /usr/local/etc/xray/domain)
               ws_tls=$(echo "{\"v\":\"2\",\"ps\":\"${u}\",\"add\":\"${DMN}\",\"port\":\"443\",\"id\":\"${id}\",\"aid\":\"0\",\"scy\":\"auto\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${DMN}\",\"path\":\"/vmess\",\"tls\":\"tls\",\"sni\":\"${DMN}\"}" | base64 -w 0)
               ws_ntls=$(echo "{\"v\":\"2\",\"ps\":\"${u}\",\"add\":\"${DMN}\",\"port\":\"80\",\"id\":\"${id}\",\"aid\":\"0\",\"scy\":\"auto\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${DMN}\",\"path\":\"/vmess\",\"tls\":\"\",\"sni\":\"\"}" | base64 -w 0)
               grpc_tls=$(echo "{\"v\":\"2\",\"ps\":\"${u}\",\"add\":\"${DMN}\",\"port\":\"443\",\"id\":\"${id}\",\"aid\":\"0\",\"scy\":\"auto\",\"net\":\"grpc\",\"type\":\"none\",\"host\":\"${DMN}\",\"path\":\"/vmess-grpc\",\"tls\":\"tls\",\"sni\":\"${DMN}\"}" | base64 -w 0)
            ...
            6) nl $D_VMESS; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_VMESS); u=$(echo "$line" | cut -d'|' -f1); id=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); quota=$(echo "$line" | cut -d'|' -f6)
               if [[ "$stat" == "ACTIVE" ]]; then
_tf9=$(mktemp) &&                    jq --arg u "$u" '(.inbounds[] | select(.protocol == "vmess")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf9" && mv "$_tf9" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
                   sed -i "${n}s/.*/$u|$id|$exp|$limit|LOCKED|$quota/" $D_VMESS
                   echo -e "${GREEN}Account $u Locked Successfully!${NC}"; fix_xray_permissions; systemctl restart xray >/dev/null 2>&1; sleep 2
               else
                   echo -e "${RED}Account is already locked!${NC}"; sleep 2
               fi;;
            7) nl $D_VMESS; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_VMESS); u=$(echo "$line" | cut -d'|' -f1); id=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); quota=$(echo "$line" | cut -d'|' -f6)
               if [[ "$stat" != "ACTIVE" ]]; then
_tf10=$(mktemp) &&                    jq --arg u "$u" --arg id "$id" '(.inbounds[] | select(.protocol == "vmess")).settings.clients += [{"id":$id,"email":$u,"level":0}]' $CONFIG > "$_tf10" && mv "$_tf10" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
                   sed -i "${n}s/.*/$u|$id|$exp|$limit|ACTIVE|$quota/" $D_VMESS
                   echo -e "${GREEN}Account $u Unlocked Successfully!${NC}"; fix_xray_permissions; systemctl restart xray >/dev/null 2>&1; sleep 2
               else
                   echo -e "${YELLOW}Account is already Active!${NC}"; sleep 2
               fi;;
            x) return;;
    ...
    while true; do header_sub
        ...
        case $opt in
            1) read -p " Username : " u; [[ -z "$u" ]] && continue
               ...
               if ! check_uuid "$id"; then continue; fi
               read -p " Expired (days): " ex; [[ -z "$ex" ]] && ex=30; read -p " Limit IP (0 for unlimited): " limit; [[ -z "$limit" ]] && limit=0; read -p " Quota Bandwidth GB (0 for unlimited): " quota; [[ -z "$quota" ]] && quota=0
               exp_date=$(date -d "+$ex days" +"%Y-%m-%d");_tf11=$(mktemp) &&  jq --arg u "$u" --arg id "$id" '(.inbounds[] | select(.protocol == "vless")).settings.clients += [{"id":$id,"email":$u,"level":0}]' $CONFIG > "$_tf11" && mv "$_tf11" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; echo "$u|$id|$exp_date|$limit|ACTIVE|$quota" >> $D_VLESS; echo "0 0" > "/usr/local/etc/xray/quota/$u"
               DMN=$(cat /usr/local/etc/xray/domain)
               ...
               ;;
            2) nl $D_VLESS; read -p "No: " n; [[ -z "$n" ]] && continue; u=$(sed -n "${n}p" $D_VLESS | cut -d'|' -f1); sed -i "${n}d" $D_VLESS;_tf12=$(mktemp) &&  jq --arg u "$u" '(.inbounds[] | select(.protocol == "vless")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf12" && mv "$_tf12" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; rm -f "/usr/local/etc/xray/quota/$u"
               echo -e "${GREEN}Account Deleted!${NC}"; fix_xray_permissions; systemctl restart xray >/dev/null 2>&1; sleep 2;;
            ...
            5) u="trial-$(tr -dc a-z0-9 </dev/urandom | head -c 5)"; echo -e " Username (Trial): ${GREEN}$u${NC}"; p="$u"; id="$p"; read -p " Duration (e.g., 10m, 1h): " dur;
               if [[ "$dur" == *m ]]; then add_str="+${dur%m} minutes"; elif [[ "$dur" == *h ]]; then add_str="+${dur%h} hours"; else add_str="+1 hours"; fi
               exp_date=$(date -d "$add_str" +"%Y-%m-%d %H:%M:%S"); limit=1; quota=0;_tf13=$(mktemp) &&  jq --arg u "$u" --arg id "$id" '(.inbounds[] | select(.protocol == "vless")).settings.clients += [{"id":$id,"email":$u,"level":0}]' $CONFIG > "$_tf13" && mv "$_tf13" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; echo "$u|$id|$exp_date|$limit|ACTIVE|$quota" >> $D_VLESS; echo "0 0" > "/usr/local/etc/xray/quota/$u"
               DMN=$(cat /usr/local/etc/xray/domain)
               ws_tls="vless://${id}@${DMN}:443?path=%2Fvless&security=tls&encryption=none&host=${DMN}&type=ws&sni=${DMN}#${u}"
               ws_ntls="vless://${id}@${DMN}:80?path=%2Fvless&security=none&encryption=none&host=${DMN}&type=ws#${u}"
               grpc_tls="vless://${id}@${DMN}:443?security=tls&encryption=none&host=${DMN}&type=grpc&serviceName=vless-grpc&sni=${DMN}#${u}"
            ...
            6) nl $D_VLESS; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_VLESS); u=$(echo "$line" | cut -d'|' -f1); id=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); quota=$(echo "$line" | cut -d'|' -f6)
               if [[ "$stat" == "ACTIVE" ]]; then
_tf14=$(mktemp) &&                    jq --arg u "$u" '(.inbounds[] | select(.protocol == "vless")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf14" && mv "$_tf14" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
            ...
            7) nl $D_VLESS; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_VLESS); u=$(echo "$line" | cut -d'|' -f1); id=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); quota=$(echo "$line" | cut -d'|' -f6)
               if [[ "$stat" != "ACTIVE" ]]; then
_tf15=$(mktemp) &&                    jq --arg u "$u" --arg id "$id" '(.inbounds[] | select(.protocol == "vless")).settings.clients += [{"id":$id,"email":$u,"level":0}]' $CONFIG > "$_tf15" && mv "$_tf15" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
    ...
    while true; do header_sub
        ...
        case $opt in
            1) read -p " Username : " u; [[ -z "$u" ]] && continue
               ...
               read -p " Expired (days): " ex; [[ -z "$ex" ]] && ex=30; read -p " Limit IP (0 for unlimited): " limit; [[ -z "$limit" ]] && limit=0; read -p " Quota Bandwidth GB (0 for unlimited): " quota; [[ -z "$quota" ]] && quota=0
               exp_date=$(date -d "+$ex days" +"%Y-%m-%d");_tf16=$(mktemp) &&  jq --arg p "$pass" --arg u "$u" '(.inbounds[] | select(.protocol == "trojan")).settings.clients += [{"password":$p,"email":$u,"level":0}]' $CONFIG > "$_tf16" && mv "$_tf16" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; echo "$u|$pass|$exp_date|$limit|ACTIVE|$quota" >> $D_TROJAN; echo "0 0" > "/usr/local/etc/xray/quota/$u"
               DMN=$(cat /usr/local/etc/xray/domain)
               ...
               ;;
            2) nl $D_TROJAN; read -p "No: " n; [[ -z "$n" ]] && continue; u=$(sed -n "${n}p" $D_TROJAN | cut -d'|' -f1); sed -i "${n}d" $D_TROJAN;_tf17=$(mktemp) &&  jq --arg u "$u" '(.inbounds[] | select(.protocol == "trojan")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf17" && mv "$_tf17" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; rm -f "/usr/local/etc/xray/quota/$u"
               echo -e "${GREEN}Account Deleted!${NC}"; fix_xray_permissions; systemctl restart xray >/dev/null 2>&1; sleep 2;;
            ...
            5) u="trial-$(tr -dc a-z0-9 </dev/urandom | head -c 5)"; echo -e " Username (Trial): ${GREEN}$u${NC}"; p="$u"; pass="$p"; read -p " Duration (e.g., 10m, 1h): " dur;
               if [[ "$dur" == *m ]]; then add_str="+${dur%m} minutes"; elif [[ "$dur" == *h ]]; then add_str="+${dur%h} hours"; else add_str="+1 hours"; fi
               exp_date=$(date -d "$add_str" +"%Y-%m-%d %H:%M:%S"); limit=1; quota=0;_tf18=$(mktemp) &&  jq --arg p "$pass" --arg u "$u" '(.inbounds[] | select(.protocol == "trojan")).settings.clients += [{"password":$p,"email":$u,"level":0}]' $CONFIG > "$_tf18" && mv "$_tf18" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"; echo "$u|$pass|$exp_date|$limit|ACTIVE|$quota" >> $D_TROJAN; echo "0 0" > "/usr/local/etc/xray/quota/$u"
               DMN=$(cat /usr/local/etc/xray/domain)
               ws_tls="trojan://${pass}@${DMN}:443?path=%2Ftrojan&security=tls&host=${DMN}&type=ws&sni=${DMN}#${u}"
               ws_ntls="trojan://${pass}@${DMN}:80?path=%2Ftrojan&security=none&host=${DMN}&type=ws#${u}"
               grpc_tls="trojan://${pass}@${DMN}:443?security=tls&host=${DMN}&type=grpc&serviceName=trojan-grpc&sni=${DMN}#${u}"
            ...
            6) nl $D_TROJAN; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_TROJAN); u=$(echo "$line" | cut -d'|' -f1); pass=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); quota=$(echo "$line" | cut -d'|' -f6)
               if [[ "$stat" == "ACTIVE" ]]; then
_tf19=$(mktemp) &&                    jq --arg u "$u" '(.inbounds[] | select(.protocol == "trojan")).settings.clients |= map(select(.email != $u))' $CONFIG > "$_tf19" && mv "$_tf19" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
            ...
            7) nl $D_TROJAN; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_TROJAN); u=$(echo "$line" | cut -d'|' -f1); pass=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); quota=$(echo "$line" | cut -d'|' -f6)
               if [[ "$stat" != "ACTIVE" ]]; then
_tf20=$(mktemp) &&                    jq --arg p "$pass" --arg u "$u" '(.inbounds[] | select(.protocol == "trojan")).settings.clients += [{"password":$p,"email":$u,"level":0}]' $CONFIG > "$_tf20" && mv "$_tf20" "$CONFIG" && chown root:nobody "$CONFIG" 2>/dev/null && chmod 640 "$CONFIG"
...
# FINAL XRAY PERMISSION REPAIR: prevents Xray from becoming OFF after install.
fix_xray_permissions

# Validate configuration and start Xray only after permissions are correct.
if [[ -x /usr/local/bin/xray && -f "$CONFIG_FILE" ]]; then
